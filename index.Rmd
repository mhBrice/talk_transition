...
>
> .image-holder[ ![:scale 1.2em](images/two.svg) Est-ce que les perturbations peuvent accélérer certaines transitions? ]
>
...
```{r trans_time, fig.width=7, fig.height=3.5}
...
- Temps continu: 
  - intervalle de temps irrégulier, avec plusieurs transitions possibles durant un intervalle
...
.right-column[
> ![:faic](thermometer-full) Température à $t_{i-1}$
...
```{r trans_diag, fig.width=9.5, fig.height=7.5}
...
```{r trans_flow, fig.width=12, fig.height=7.5}
...
d_grad <- c(0, .2, .5, 1)
c_grad <- seq(0.25, 1, len = 4)
df <- expand.grid(sTP = tp_grad, logging1 = d_grad,
                  CMI = mixed_mean[[2]],
                  DRAIN = mixed_mean[[3]],
...
col_bb <- alpha("#158282", c_grad)
col_tt <- alpha("#D43650", c_grad)
...
##### Différentes fréquences de coupes modérées
```{r steady_state11, fig.width=9.2, fig.height=6.5}
plot_SS(d = 1)
```
...
##### Différentes fréquences de coupes modérées 
```{r steady_state12, fig.width=9.2, fig.height=6.5}
plot_SS(d = 2)
```
...
##### Différentes fréquences de coupes modérées
```{r steady_state13, fig.width=9.2, fig.height=6.5}
plot_SS(d = 3)
```
...
##### Différentes fréquences de coupes modérées
```{r steady_state14, fig.width=9.2, fig.height=6.5}
plot_SS(d = 4)
```
...
```{r prep_SS2}
df <- expand.grid(sTP = tp_grad, logging2 = d_grad,
                  CMI = mixed_mean[[2]],
                  DRAIN = mixed_mean[[3]],
                  PH_HUMUS = mixed_mean[[4]])
...
##### Différentes fréquences de coupes majeures

```{r steady_state21, fig.width=9.2, fig.height=6.5}
plot_SS(d = 1)
```
...
##### Différentes fréquences de coupes majeures
```{r steady_state22, fig.width=9.2, fig.height=6.5}
plot_SS(d = 2)
```
...
##### Différentes fréquences de coupes majeures
```{r steady_state23, fig.width=9.2, fig.height=6.5}
plot_SS(d = 3)
```
...
##### Différentes fréquences de coupes majeures
```{r steady_state24, fig.width=9.2, fig.height=6.5}
plot_SS(d = 4)
```
...
Temps de turnover ![:faic](arrow-right) temps de séjour dans un état avant de faire une transition
...
barplot_index(index = as.matrix(sojs), bars = 1, 
              ylab = "Temps de turnover (années)", ylim = c(0,200))
...
.alert[Oui, mais difficilement sans perturbations.]
